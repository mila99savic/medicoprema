  ...
  <div class="evidencija-clanova-container">
    <div class="evidencija-clanova">
      ...
      <div id="select">
        ...
        <h5
          style="text-align:center; font-family:'Times New Roman', Times, serif; flex:1"
        >Evidencija članova</h5>
      ...
      <el-table style="width:100%" :data="this.tableData.filter(data => !search || data.name.toLowerCase().includes(search.toLowerCase()))">
      ...
      <div class="dugme-dodaj">
        <el-button
          round
          size="mini"
          style="height:50px; width:130px; color:white; margin-right:5px; border-color:rgba(24, 102, 89, 0.925); background-color:rgba(24, 102, 89, 0.925);"
          @click="dodajClana"
        >Dodaj zaposlenog</el-button>
      </div>
    </div>
    <form-dodaj-zaposlenog
      v-if="this.showComp == 'zaposleni'"
      @zatvoriDodavanjeClana="zavrsiDodavanje"
    ></form-dodaj-zaposlenog>
  </div>
</template>
...
import {
  ...
  EMPLOYED_USER_TYPE,
  REGULAR_USER_TYPE
} from "../services/authFetch";
export default {
  components: {  FormDodajZaposlenog },
  data() {
    return {
      tableData: [],
      showComp: "",
  ...
  methods: {
    ...
    //  const response = await apiFetch('GET', destinationUrl + "/user/users");
    //  console.log(response)
    //     const ljudi = await response.json();
    //     this.tableData = ljudi;

    //},
    loadDataTable() {
      apiFetch("GET", destinationUrl + "/user/users").then(result => {
        console.log(result);
    ...
    setUser(event) {
      ...
      if (this.user == usertypes[EMPLOYED_USER_TYPE]) {
        apiFetch("GET", destinationUrl + "/User/GetAllEmployed").then(
          result => {
            this.handleResponse(result);
          }
        );
      } else if (this.user == usertypes[REGULAR_USER_TYPE]) {
        apiFetch("GET", destinationUrl + "/User/GetAllRegularUsers").then(
          result => {
            this.handleResponse(result);
          }
        );
